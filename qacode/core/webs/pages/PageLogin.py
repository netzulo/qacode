...
class PageLogin(PageBase):
    ...
    def __init__(self, bot, url, by=By.CSS_SELECTOR, selectors=[],
                 go_url=True):
        super(PageLogin, self).__init__(bot, url, by, selectors, go_url)
        if len(selectors) != 3:
            raise PageException(
                "Can't instance PageLogin elements if not have 3 selectors"
            )
    ...
    def login(self, username, password, is_login_now=True):
        ...
        try:
            ...
            raise PageException(message_error.format("send_keys", username,
                                                     "txt_username"))
        try:
            self.txt_password.send_keys(password)
        except Exception as err:
            raise PageException(message_error.format("send_keys", password,
                                                     "txt_password"))
        if is_login_now:
            try:
                self.btn_login.click()
            except Exception as err:
                raise PageException(message_error.format("click", password,
                                                         "btn_login"))
