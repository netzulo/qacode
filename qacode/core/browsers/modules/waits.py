...
class ModuleWaits(object):
    ...
    @staticmethod
    def ele_wait_invisible(driver, selector, locator=By.CSS_SELECTOR,
                           timeout=0):
        ...
        try:
            ...
            raise Exception("Fails at wait for invisible element")
        return element
    ...
    def ele_wait_visible(driver, element, timeout=0):
        ...
        if element is None:
            raise Exception("Can't wait visible if element is None")
        driver_wait = WebDriverWait(driver, timeout)
        try:
            element = driver_wait.until(EC.visibility_of(element))
        except Exception:
            raise Exception("Fails at wait for visible element")
        return element

    @staticmethod
    def ele_wait_text(driver, selector, text,
        ...
        try:
            ...
            raise Exception("Fails at wait for element text")
    ...
    def ele_wait_value(driver, selector, value,
        ...
        try:
            ...
            raise Exception("Fails at wait for element value")
