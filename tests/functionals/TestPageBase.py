...
class TestPageBase(TestInfoBot):
    ...
    def test_001_page_base_instance(self):
        try:
            PageBase(self.bot, cfg['TEST_UNITARIES']['url'])
            assert (cfg['TEST_UNITARIES']['url']
                    in self.bot.curr_driver.current_url)
        except PageException as err:
            raise Exception(err)

    def test_002_page_base_instance_go_url_False(self):
        try:
            PageBase(self.bot, cfg['TEST_UNITARIES']['url'], go_url=False)
            assert (cfg['TEST_UNITARIES']['url']
                    not in self.bot.curr_driver.current_url)
        except PageException as err:
            raise Exception(err)
